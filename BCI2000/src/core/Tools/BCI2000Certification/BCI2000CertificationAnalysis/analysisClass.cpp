...
bool analysis::open(string file, Tasks &taskTypes)
{
    ...
    fName = file;
	int nTasks = 0;
	thisTask.setTaskName("Unknown", err);
    ...
    if (!dFile->IsOpen())
    {
        ...
        mIsOpen = false;
		thisTask.setTaskName("",err);
		return false;
    }
	//get useful parameters
    if (!dFile->Parameters()->Exists("SampleBlockSize") ||
		!dFile->Parameters()->Exists("SamplingRate") ||
		!dFile->Parameters()->Exists("SubjectName")){
	...
	for (size_t i = 0; i < taskTypes.size() && !taskFound; i++)
    {
		...
		nameFound = (taskTypes[i].getTaskName().toStdString() == SubjectName);
        ...
        {
            statesFound = (isMember(stateNames, taskTypes[i].getSstates[s]) && statesFound);
		}
		*/
        ...
        if (statesFound && parmsFound && nameFound)
		{
			...
			thisTask.setSampleRate(sampleRate, err);
			thisTask.setBlockSize(blockSize, err);
			taskFound = true;
            nTasks++;
        }
	}
...
bool analysis::doThreshAnalysis(double threshTmp)
{
	...
	if (thisTask.getAmpFlag())
    {
		...
		ampStats = doThreshAnalysis(thisTask.getAmpCh());
		...
		checkDroppedSamples(thisTask.getAmpCh());
	}

    basicStats dAmpStats;
	if (thisTask.getDAmpFlag())
    {
        //the amp analysis does not depend on any specific state, so just do the
        //analysis on the specified channel based on the latency from the block start
		dAmpStats = doThreshAnalysis(thisTask.getDAmpCh());
	...
	if (thisTask.getVidFlag())
    {
        //do the video analysis on the channel and state specified
		vidSystemStats = doThreshAnalysis(thisTask.getVidCh(), thisTask.getVidState().toStdString(), thisTask.getVidVals());
        vidSystemStats.taskName = "VidSys  ";
        latencyStats.push_back(vidSystemStats);

		checkDroppedSamples(thisTask.getVidCh());
    }

    //do the analysis on each state for the audio
    basicStats audSystemStats;
	if (thisTask.getAudFlag())
    {
        audSystemStats = doThreshAnalysis(thisTask.getAudCh(), thisTask.getAudState().toStdString(), thisTask.getAudVals());
        audSystemStats.taskName = "AudSys  ";
        latencyStats.push_back(audSystemStats);

		checkDroppedSamples(thisTask.getAudCh());
    }
    ...
    vector<double> metronomeDiff, metronomeDiffSW;
	...
	{
		t2 = static_cast<unsigned short>( states["StimulusTime"][i] );
		t1 = static_cast<unsigned short>( states["SourceTime"][i] );
		tans = t2-t1;
		procLat.push_back(tans);
	}
	for (int i = mIgnoreDur; i < nSamples; i += blockSize){
		t2 = static_cast<unsigned short>( states["SourceTime"][i] );
		t1 = static_cast<unsigned short>( states["SourceTime"][i-blockSize] );
		tans = t2-t1;
		metronomeDiffSW.push_back(tans);
	}

	if(thisTask.getDAmpFlag()){
		for (unsigned int i = 1; i < dAmpStats.samples.size()-1; i++){
			metronomeDiff.push_back(1000*(dAmpStats.samples[i+1] - dAmpStats.samples[i])/sampleRate);
		}
	}
	else if (thisTask.getAmpFlag()){
		for (unsigned int i = 1; i < ampStats.samples.size()-1; i++){
			metronomeDiff.push_back(1000*(ampStats.samples[i+1] - ampStats.samples[i])/sampleRate);
		}
	}
	double blockDur = double(blockSize)*1000/double(sampleRate);
	for (unsigned int i = 0; i < metronomeDiff.size(); i++)
		metronomeDiff[i] = (metronomeDiff[i] - blockDur);

	for (unsigned int i = 0; i < metronomeDiffSW.size(); i++)
		metronomeDiffSW[i] = (metronomeDiffSW[i] - blockDur);
	removeNPercentile(&metronomeDiff, 3.0);
	removeNPercentile(&metronomeDiffSW, 3.0);
	removeNPercentile(&procLat, 3.0);
    ...
    basicStats metronome, metronomeSW;
	...
	metronomeSW.mean = vMean(&metronomeDiffSW);
	metronomeSW.median = vMedian(&metronomeDiffSW);
	metronomeSW.max = vMax(&metronomeDiffSW);
	metronomeSW.min = vMin(&metronomeDiffSW);
	metronomeSW.std = vStd(&metronomeDiffSW);
	metronomeSW.vals = metronomeDiffSW;

    metronomeSW.taskName = "JitterSW";
    metronomeSW.desc = "SourceTime[t+1] - SourceTime[t]";

    //add these to the task stats array
    latencyStats.push_back(metronome);
	latencyStats.push_back(metronomeSW);
	...
	if (thisTask.getDAmpFlag() && thisTask.getVidFlag() && dAmpStats.mean != -1 && vidSystemStats.mean != -1)
    {
		...
		for (size_t i = 0; i < vidOutputStats.vals.size(); i++)
			vidOutputStats.vals[i] -= (spMean + dAmpStats.mean);
	...
	if (thisTask.getDAmpFlag() && thisTask.getAudFlag() && dAmpStats.mean != -1 && audSystemStats.mean != -1)
    {
		...
		for (size_t i = 0; i < audOutputStats.vals.size(); i++)
			audOutputStats.vals[i] -= (spMean + dAmpStats.mean);
...
basicStats analysis::doThreshAnalysis(int chNum)
{
	...
	for (int s = mIgnoreDur; s < nSamples-blockSize; s+=blockSize){
		for (int ss = 0; ss < blockSize; ss++){
			if (dSig[s+ss] >= std){
				tDiff.push_back(1000*ss/sampleRate);
				samples.push_back(s+ss);
				ss += blockSize+1;
				break;
			}
		}
    ...
    if (tDiff.size() > 0)
    {
        ...
        tmpStat.vals = tDiff;
		tmpStat.samples = samples;
    }

    stringstream str;
    str << "Ch " << chNum <<" v. blocks";
	tmpStat.desc = str.str().c_str();
...
basicStats analysis::doThreshAnalysis(int chNum, string stateName, vector<int> stateVal)
{
	...
	for (int s = mIgnoreDur; s < nSamples-1; s++){
		if (dSig[s] > std){
	...
	for (size_t i = 0; i < stateChangePos.size()-1; i++){
		for (size_t j = lastPos; j < signalChangePos.size(); j++){
			if (signalChangePos[j] >= stateChangePos[i] && signalChangePos[j] < stateChangePos[i+1]){
				...
				unsigned short t1 = static_cast<unsigned short>( states["SourceTime"][stateChangePos[i]] );
				unsigned short t2 = static_cast<unsigned short>( states["SourceTime"][stateChangePos[i]-blockSize] );
				tmpStat.jitter.push_back(t1-t2 );
			}
			if (signalChangePos[j] > stateChangePos[i+1]) j = signalChangePos.size();
		}
	}
...
bool analysis::print(FILE * out, vector<basicStats*> minReqs, int num)
{
	...
	for (size_t t = 0; t < latencyStats.size(); t++)
    {
        ...
        {
			if (tmpTask.taskName.trimmed().toLower().compare(minReqs[a]->taskName.trimmed().toLower()) == 0)
			//if (tolower(strtrim(tmpTask.taskName)) == tolower(strtrim(minReqs[a]->taskName)))
            {
                tfound = 1;
				if (tmpTask.mean <= (minReqs[a]->mean)){
					if (tmpTask.taskName.trimmed().toLower().compare("jitter")==0 ||
						tmpTask.taskName.trimmed().toLower().compare("jittersw")==0 ||
							tmpTask.mean >= 0){
						fprintf(out, "%s\t","pass");
					}
    ...
    if (ok)
    {
        ...
        cout<<SubjectName<< " passed the minimum BCI2000 requirements."<<endl;
    }
    else
    {
        fprintf(out, "\tOverall\tFAILED\n");
        cout<<SubjectName<< " did not pass the minimum BCI2000 requirements."<<endl;
    }
    fprintf(out,"\n\n");
	return ok;
}
...
bool analysis::exportData(string expfile)
{
	...
	for (size_t i = 0; i < latencyStats.size(); i++)
    {
        ...
        if (tmpTask.vals.size() == 0)
            continue;
		o<<tmpTask.taskName.toStdString();
        o.precision(4);
